...
banish.prototype.run = function(fn){
	...
	var fn = (function(f){
		return function(){
			var args = arguments;
			// remove lockfile
			fs.unlink(self.opts.lockfile, function(){
				if (typeof f === 'function') f.apply(self, args);
			});
		};
	})(fn);

	self.pathname = url.parse(self.opts.url).pathname;

	// create lockfile
	fs.writeFile(self.opts.lockfile, Date.now(), function(){
...
banish.prototype.resume = function(fn){
	...
	// ensure dir exists
	fs.mkdir(self.opts.dest, { recursive: true }, function(err){
		if (err) return fn(err);
...
banish.prototype.collect = function(fn){
	...
	needle.request("GET", url.resolve(self.cache.url, "ghost/api/v0.1/posts/"), {
		...
		(function(write){
			...
			switch (resp.headers['content-type'].split(/;/g).shift().toLowerCase()) {
				case "text/html":
					...
					// fix css urls, because ghost fucks it up when not installed in root
					if (self.pathname !== '/') content = content.replace('url(/content/', 'url('+(self.pathname.replace(/\/$/,''))+'/content/');
					...
					try {
						...
						$('[style]').each(function(i,e){
							$(e).attr("style").replace(/url\(([^)]+)\)/g, function(m, p){
								self.queue(url.resolve(u, p));
							});
						});
					...
					}, htmlnano.presets.safe).then(function(content) {
						write(content.replace(/target="[^"]+"\s*/g,'').replace(/<script type="application\/ld\+json">([^<]+)<\/script>/,function(a,b){
							// make script-json small
							return '<script type="application/ld+json">'+JSON.stringify(JSON.parse(b))+'</script>';
						}).replace(/>\s+</g,'><')))
					});
